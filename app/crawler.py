...
class PaginatedCrawler:
    ...
    async def navigate_to_next_page(self) -> bool:
        ...
        try:
            ...
            pagination_elements = await self.page.query_selector_all(self.config.pagination_selector)
            ...
            if hasattr(self.config, 'pagination_original_content') and self.config.pagination_original_content:
                original_content = self.config.pagination_original_content.strip().lower()
                ...
                if not selected_element and pagination_elements:
                    self.logger.warning(f"No pagination element found with content matching '{original_content}', using first available")
    ...
    async def crawl(self, custom_extractor: Optional[Callable] = None) -> List[Dict[str, Any]]:
        ...
        while True:
            ...
            self.logger.info(f"Extracted {len(page_data)} items from page {self.current_page}")
