...
async def demo_user_element_selection():
    ...
    demo_url = input("🌐 Enter URL to configure (or press Enter for quotes demo): ").strip()
    if not demo_url:
...
async def demo_sophisticated_workflows():
    ...
    print("🔗 Click element → Navigate to sub-page → Extract data → Return to main page")
    ...
    workflow_steps = workflow_builder.add_click_and_extract(
        ...
        click_selector=".author",  # Click on author name
        extract_fields=["author_bio", "author_born_date", "author_born_location"],
        description="Click author name → Navigate to author page → Extract bio → Return to quotes"
...
async def demo_real_world_scenario():
    ...
    print(f"\n📊 Extraction Plan:")
    print(f"Base fields: {len([s for s in config.selections if s.element_type == 'data_field'])} data points")
    print(f"Workflow steps: {len(config.workflows)} sophisticated extraction steps")
    print(f"Total possible fields per item: ~{len([s for s in config.selections if s.element_type == 'data_field']) + sum(len(w.extract_fields or []) for w in config.workflows)}")
