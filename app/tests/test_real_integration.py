...
class TestLocalHTMLIntegration:
    ...
    async def test_basic_extraction_from_local_html(self):
        ...
        config = CrawlerConfiguration(
            ...
            selections=[
                ...
                ElementSelection("price", ".product-price", "data_field", "Product price"),
                ElementSelection("description", ".product-description", "data_field", "Product description")
            ],
            workflows=[],
            max_pages=1
        )
    ...
    async def test_pagination_with_local_html(self):
        ...
        assert len(results) == 6, f"Expected 6 products from 2 pages, got {len(results)}"
        
        # Verify we have products from both pages
        page_1_products = ["Wireless Headphones", "Bluetooth Speaker", "Smart Watch", "USB-C Cable"]
        ...
        for expected_title in page_1_products + page_2_products:
            assert expected_title in extracted_titles, f"Missing product: {expected_title}"
    ...
    async def test_click_workflow_with_local_html(self):
        ...
        config = CrawlerConfiguration(
            ...
            selections=[
                ...
                ElementSelection("detailed_title", ".product-title", "data_field", "Detailed title"),
                ElementSelection("detailed_price", ".product-price", "data_field", "Detailed price"),
    ...
    async def test_error_handling_missing_elements(self):
        ...
        config = CrawlerConfiguration(
            ...
            selections=[
                ElementSelection("items", ".product", "items_container", "Products"),
                ElementSelection("title", ".product-title", "data_field", "Title"),
                ElementSelection("nonexistent", ".does-not-exist", "data_field", "Missing field"),
                ElementSelection("also_missing", ".another-missing", "data_field", "Another missing")
            ],
            workflows=[],
            max_pages=1
        )
...
class TestRealWebsiteIntegration:
    ...
    async def test_quotes_toscrape_basic_extraction(self):
        ...
        assert any("Einstein" in author for author in extracted_authors), "Should have Einstein quotes"
    ...
    async def test_quotes_toscrape_pagination(self):
        ...
        config = CrawlerConfiguration(
            ...
            workflows=[],
            pagination_config=ElementSelection("next", "li.next a", "pagination", "Next page"),
        ...
        assert len(results) >= 20, f"Expected at least 20 quotes from 3 pages, got {len(results)}"
        ...
        assert len(unique_texts) >= len(results) * 0.8, "Too many duplicate quotes - pagination may not be working"
    ...
    async def test_httpbin_different_content_types(self):
        ...
        assert 'slideshow' in content or 'json' in content.lower(), f"Unexpected JSON content: {content}"
...
class TestAdvancedWorkflowsRealBrowser:
    ...
    async def test_workflow_click_navigation_real_browser(self):
        ...
        config = CrawlerConfiguration(
            ...
            selections=[
                ElementSelection("items", ".product", "items_container", "Products"),
                ElementSelection("title", ".product-title", "data_field", "Basic title"),
                ElementSelection("price", ".product-price", "data_field", "Basic price"),
    ...
    async def test_element_visibility_and_clickability(self):
        ...
        successful_extractions = [r for r in results if any(v is not None for k, v in r.data.items() if 'workflow' in k or k in ['detailed_title', 'rating'])]
...
class TestRealWorldScenarios:
    ...
    async def test_quotes_author_detail_workflow(self):
        ...
        workflow_builder.add_click_and_extract(
            ...
            click_selector=".author",  # Author names are clickable on quotes.toscrape.com
        ...
        config = CrawlerConfiguration(
            ...
            selections=[
                ...
                ElementSelection("author_bio", ".author-description", "data_field", "Author biography"),
                ElementSelection("author_birth", ".author-born-date", "data_field", "Birth date")
            ],
            workflows=workflow_builder.build(),
            max_pages=1
        )
        ...
        assert len(results_with_bio) >= 1, "At least one quote should have author bio extracted"
    ...
    async def test_performance_with_real_browser(self):
        ...
        valid_results = [r for r in results if r.data.get('text') and r.data.get('author')]
        assert len(valid_results) >= len(results) * 0.8, "Most extractions should be successful"
