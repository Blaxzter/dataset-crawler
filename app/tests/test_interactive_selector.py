...
class TestCrawlerConfiguration:
    ...
    def test_crawler_configuration_creation_full(self):
        ...
        pagination_config = ElementSelection(
            "next", ".next", "pagination", "Next page"
        )
...
class TestInteractiveSelector:
    ...
    @patch('app.interactive_selector.async_playwright')
    ...
    async def test_get_configuration_with_navigation_workflow_generation(self):
        ...
        page_selections = {
            'https://test.com': [
                {'name': 'nav_link', 'element_type': 'navigation'}
            ],
        ...
        mock_page.evaluate = AsyncMock(side_effect=[config_data, page_selections, 'https://test.com'])
        mock_page.url = "https://test.com"
        selector.page = mock_page
    ...
    def test_preview_configuration(self, capsys):
        ...
        workflows = [
            WorkflowStep("step1", "click", ".link", "Test step", ["detail"])
        ]
...
class TestConfigurationComplexity:
    ...
    def test_multi_step_workflow_configuration(self):
        ...
        workflows = [
            WorkflowStep(
                "get_details", "click", ".detail-link", 
                "Get product details", ["detail"]
            ),
            WorkflowStep(
                "get_reviews", "open_new_tab", ".review-link",
                "Get reviews in new tab", ["review"]
            )
        ]
