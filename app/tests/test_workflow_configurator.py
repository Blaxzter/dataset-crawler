...
class TestWorkflowConfigurator:
    ...
    @patch('os.makedirs')
    def test_ensure_config_directory_creates_directory(self, mock_makedirs, mock_exists):
    ...
    def test_list_configurations_with_data(self, capsys):
        ...
        config = CrawlerConfiguration(
            ...
            workflows=[
                WorkflowStep("step1", "click", ".link", "Test step")
            ]
        )
...
class TestWorkflowConfiguratorIntegration:
    ...
    @patch('app.workflow_configurator.InteractiveSelector')
    async def test_create_interactive_configuration(self, mock_selector_class):
        """Test creating interactive configuration"""
        # Setup mocks
        mock_selector = AsyncMock()
        mock_selector_class.return_value.__aenter__ = AsyncMock(return_value=mock_selector)
        ...
        # Test the method
        result = await self.configurator.create_interactive_configuration("https://test.com", "test_config")
        ...
        # Verify method calls
        mock_selector.start_selection_session.assert_called_once_with("https://test.com")
    ...
    @patch('app.workflow_configurator.AdvancedCrawler')
    async def test_test_configuration(self, mock_crawler_class):
        ...
        # Mock crawl results
        mock_results = [
            Mock(data={'title': 'Test Product'})
        ]
        mock_crawler.crawl_with_workflows = AsyncMock(return_value=mock_results)
        mock_crawler.get_extraction_summary = Mock(return_value={
            'total_items': 1,
            'unique_sources': 1,
            'workflow_usage': 0
        })
    ...
    async def test_run_full_crawl(self, mock_crawler_class):
        ...
        # Test the method
        result = await self.configurator.run_full_crawl("full_crawl_test", "output.json")
    ...
    @patch('app.workflow_configurator.AdvancedCrawler')
    async def test_run_full_crawl_exception(self, mock_crawler_class):
        """Test running full crawl when crawler raises exception"""
        # Create test configuration
        config = CrawlerConfiguration(
            name="error_test",
            base_url="https://test.com",
        ...
        self.configurator.configurations["error_test"] = config
        
        # Setup crawler mock to raise exception
        mock_crawler = AsyncMock()
        mock_crawler_class.return_value.__aenter__ = AsyncMock(return_value=mock_crawler)
        mock_crawler_class.return_value.__aexit__ = AsyncMock(return_value=None)
        mock_crawler.crawl_with_workflows = AsyncMock(side_effect=Exception("Crawl failed"))
        
        # Test the method
        result = await self.configurator.run_full_crawl("error_test")
...
class TestWorkflowConfiguratorProgrammaticWorkflows:
    ...
    def test_create_workflow_with_all_field_types(self):
        ...
        data_field_selections = [s for s in config.selections if s.element_type == "data_field"]
...
class TestConfigurationFileManagement:
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('json.load')
    def test_load_configuration_file_path_handling(self, mock_json_load, mock_file, mock_path_join):
