...
class TestAdvancedCrawlerIntegration:
    ...
    def test_workflow_builder_to_configuration_integration(self):
        ...
        workflows = (
            builder.add_click_and_extract("step1", ".link", ["field1"], "Test step")
            .add_new_tab_extraction("step2", ".external", ["field2"], "External step")
            .build()
        )
...
class TestConfigurationFileIntegration:
    ...
    def test_save_and_load_configuration_roundtrip(self):
        ...
        assert (
            loaded_config.pagination_config.name
            == original_config.pagination_config.name
        )
        assert (
            loaded_config.pagination_config.selector
            == original_config.pagination_config.selector
        )
...
class TestEndToEndWorkflow:
    ...
    def test_complex_workflow_configuration_structure(self):
        ...
        selections = [
            ...
            # Detail page fields
            ElementSelection(
                "description", ".full-description", "data_field", "Product description"
            ),
            ElementSelection(
                "specifications", ".spec-table", "data_field", "Technical specs"
            ),
            # Review page fields
            ElementSelection(
                "reviews", ".review-text", "data_field", "Customer reviews"
            ),
            ElementSelection(
                "rating", ".average-rating", "data_field", "Average rating"
            ),
            # Navigation elements
            ElementSelection(
                "detail_link", ".product-title a", "navigation", "Product detail link"
            ),
            ElementSelection(
                "review_link", ".reviews-tab a", "navigation", "Reviews tab link"
            ),
            # Pagination
            ElementSelection(
        ...
        pagination_config = next(
            (s for s in selections if s.element_type == "pagination"), None
        )
        ...
        all_field_names = {
            s.name for s in config.selections if s.element_type == "data_field"
        }
        ...
        for workflow in config.workflows:
            if workflow.extract_fields:
                for field in workflow.extract_fields:
                    assert (
                        field in all_field_names
                    ), f"Workflow references unknown field: {field}"
...
class TestConfigurationValidation:
    ...
    def test_workflow_field_reference_validation(self):
        ...
        selections = [
            ...
            ElementSelection(
                "existing_field", ".field", "data_field", "Existing field"
            ),
        ]
