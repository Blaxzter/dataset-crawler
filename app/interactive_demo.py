...
class InteractiveCrawlerDemo:
    ...
    async def _demo_interactive_selection(self):
        ...
        print(
            "2. Select 'Items Container' mode and click on the container holding repeated items"
        )
        ...
        config = await self.configurator.create_interactive_configuration(
            url, config_name
        )
    ...
    async def _demo_test_configuration(self):
        ...
        if config_name:
            ...
            if success:
                continue_crawl = (
                    input("\n✅ Test successful! Run full crawl? (y/N): ")
                    .strip()
                    .lower()
                )
                if continue_crawl == "y":
    ...
    async def _demo_advanced_workflows(self):
        ...
        config = CrawlerConfiguration(
            ...
            selections=[
                ElementSelection(
                    "articles", ".article-item", "items_container", "News articles"
                ),
                ElementSelection(
                    "headline", ".headline", "data_field", "Article headline"
                ),
                ElementSelection(
                    "summary", ".summary", "data_field", "Article summary"
                ),
                ElementSelection(
                    "author_name", ".author-info .name", "data_field", "Author name"
                ),
                ElementSelection(
                    "full_article",
                    ".article-content",
                ...
                ElementSelection(
                    "author_bio", ".author-biography", "data_field", "Author biography"
                ),
                ElementSelection(
                    "related_articles",
                    ".related-article",
        ...
        test_it = (
            input("\n🧪 Would you like to test this configuration? (y/N): ")
            .strip()
            .lower()
        )
        if test_it == "y":
